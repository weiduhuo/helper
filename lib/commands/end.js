...
module.exports = async function main({ input, offline }) {
  ...
  await files.reduce((seq1, file) => seq1.then(async () => {
    ...
    if (!offline) await fs.outputJson(file.path, results, { spaces: 2 });
  }), Promise.resolve());
  const onairFile = path.join(input, 'onair.json');
  await fs.outputJson(onairFile, onairs, { spaces: 2 });
  console.log(`Onair data is outputed to ${onairFile}`);
};
